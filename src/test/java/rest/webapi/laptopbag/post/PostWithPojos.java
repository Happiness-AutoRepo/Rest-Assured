...
public class PostWithPojos {
	...
	public void PostActionUsingPOJO() throws URISyntaxException{
		URI postURI=new URI(RestHelpers.buildURI(Methods.ADD));
		...
		given().headers(RestHelpers.getHeaders(HeaderType.JSON))
		.and().body(laptop)
		.when().post(postURI)
	...
	public void PostActionUsingPOJO2() throws URISyntaxException{
		...
		Laptop returnLaptop=given().headers(RestHelpers.getHeaders(HeaderType.JSON))
